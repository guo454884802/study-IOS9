...
//        NSNotificationCenter.defaultCenter().addObserver(self, selector: "thumImageGet:", name: MPMoviePlayerThumbnailImageRequestDidFinishNotification, object: play1.moviePlayer)
    ...
    func saveData(){
        ...
        let relativePath = "vid\(random).mp4"
        let tempPath = documentsDirectory.stringByAppendingPathComponent(relativePath)
        let success = videoData?.writeToFile(tempPath, atomically: true)
        if (success != nil){
            print("保存成功")
            print(tempPath)
        }else{
        ...
        if let managedObjectContext = (UIApplication.sharedApplication().delegate as? AppDelegate)?.managedObjectContext{
            ...
            video.name = "11"
            video.time = "2015年11月22日"
            video.videoPath = relativePath
